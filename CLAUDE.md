...
`scripts/oscar_pipeline.sh <pairs.json>` downloads full-res images from the MJ CDN (`download_images.py`, auth via Arc browser cookies), upscales 4x with Real-ESRGAN (`~/tools/realesrgan/`), and converts with `convert_cmyk.py` (LittleCMS via Pillow, relative colorimetric + BPC, about a minute per issue) to two CMYK folders: `HH Links/` (SWOP, US press) and `KOPA Links/` (PSO, Euro press). It embeds the ICC file exactly as Photoshop does (original bytes, header intent byte set to relative colorimetric); re-serializing through LittleCMS makes macOS render the files visibly differently. `--photoshop` runs the old `convert_both.jsx` path instead. Then in InDesign, `place_oscar_pairs.jsx` places pairs + prompts into the template and, in the same run and undo step, chains `rag_prompts.jsx` (new pages only) and `place_qr_codes.jsx`, which creates the QR PNGs (Python `qrcode` via `do shell script`, medium error correction, no white border so the code sits flush in its frame), links and places them in `<doc folder>/QR Codes/`. The .indd must be saved in the issue folder. All three stay runnable on their own for touch-ups; `rag_selected.jsx` re-rags just the selected frame(s) near their current hand-set width (±1/4"; a partner prompt keeps its rag, QR codes move with their prompts, non-prompt frames are ragged in place).
